import { ThreeRuntime } from './three-runtime.js';
window.CAVE_API_BASE = window.CAVE_API_BASE || "https://iamemanuele.pythonanywhere.com";
window.CAVE_OVERLAY = (typeof window.CAVE_OVERLAY === 'boolean') ? window.CAVE_OVERLAY : false;
;(function () {
    'use strict';
    // ------------------------------------------------------------
    // CONFIG
      ...
      if (!host.dataset.shellInitialized) {
        host.innerHTML = `
          <div class="hdr">
            ...
            <span class="hint">🔄 Updates when you change duration, selection or simulator inputs.</span>
          </div>
...
function updateTicker(){
  ...
  el.textContent = 'Waiting for events…';
    ...
    async function fetchUserNFTs() {
      ...
      if (statusEl) {
        statusEl.textContent = 'Loading goblins from the API…';
      }
...
async function initThreeRuntime() {
  // aspetta DOM pronto (così #game-canvas esiste sicuro)
  if (document.readyState === 'loading') {
    await new Promise(res => document.addEventListener('DOMContentLoaded', res, { once: true }));
  }

  const State = window.__GOBLIN_DEX_STATE__;
  if (!State) throw new Error('State not found on window.__GOBLIN_DEX_STATE__');
  const canvas = State.ui?.canvas || document.getElementById('game-canvas');
  if (!canvas) throw new Error('Canvas #game-canvas not found');
  if (State.engine && State.engine.__isThreeRuntime) return;
  State.engine = new ThreeRuntime(canvas);
  State.engine.__isThreeRuntime = true;
  await State.engine.init();
}
initThreeRuntime().catch(err => {
  console.error('ThreeRuntime init failed:', err);
});
