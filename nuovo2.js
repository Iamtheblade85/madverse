;(function () {
		    ...
		    const State = {
		      ...
		      runtime: {
		        ...
		        listMode: 'all',   // all|selected|ready|tired
		        sortMode: 'best',  // best|dp|loot|res|spd|acc
		        bestPickOrder: ['spd','loot','res','acc'],
		        openDetailsId: null, // only one card expanded
		      },
		      drop: {
		        current: null,
		        serverKey: '',
			...
			// ------------------------------------------------------------
			// AUTH (iframe) — receive from parent
			// ------------------------------------------------------------
			window.userData = window.userData || {};
			window.addEventListener('message', (ev) => {
			  if (ev.origin !== window.location.origin) return;
			  const msg = ev.data;
			  if (!msg || msg.type !== 'CC_AUTH') return;
			  window.userData.wax_account = msg.wax_account || '';
			  if (msg.user_id != null)   window.userData.user_id = msg.user_id;
			  if (msg.usx_token != null) window.userData.usx_token = msg.usx_token;
			  try { syncUserFromStorage(); } catch {}
			  console.log('[nuovo] auth received', window.userData);
			});
		    ...
		    function normalizeDrop(d){
		      ...
		      // ✅ accept backend shape: sometimes only event_id exists
			...
			// --- numeric formatting (max 3 decimals for forecast UI) ---
			function fmt3(v){
			  const n = Number(v);
			  if (!Number.isFinite(n)) return '—';
			  // max 3 decimali, rimuove zeri finali
			  return n.toFixed(3).replace(/\.?0+$/,'');
			}
			function fmt3Smart(v){
			  const n = Number(v);
			  if (!Number.isFinite(n)) return '—';
			  // se è grande, compatto; se è piccolo, max 3 decimali
		...
		function pickObject(payload, ...keys){
		  ...
		  return payload;
		    ...
		    function nominalCostPerGoblinForMinutes(mins){
		      const m = toNumber(mins);
		      if (!m) return 0;
		      // Stima lineare semplice: 24h ≈ 100 DP base
		      return (m / 1440) * 100;
		    }
		...
		// ============================================================
		// STAKING REPORT — UX/UI + FETCH  (POOLS + EXPAND/COLLAPSE)
		// ============================================================
		...
		function setStakingMeta({account, startedAtUtc, cacheLabel}){
		  if (State.ui.stakingAccount) State.ui.stakingAccount.textContent = account || '—';
		  if (State.ui.stakingStartedAt) State.ui.stakingStartedAt.textContent = startedAtUtc ? fmtUtc(startedAtUtc) : '—';
		  if (State.ui.stakingCache) State.ui.stakingCache.textContent = cacheLabel || '—';
		}
		...
		function renderStakingError(message){
		  ...
		  host.innerHTML = `
		    <div class="staking-card">
		      <div class="title">⚠️ Staking report unavailable</div>
		      <div style="opacity:.9;line-height:1.45">${safe(message || 'Could not load staking report right now.')}</div>
		      <div style="margin-top:10px;display:flex;gap:8px;flex-wrap:wrap">
		        <button class="btn-primary" type="button" id="btn-staking-retry">🔄 Try again</button>
		      </div>
		    </div>
		  `;
		...
		function fmtDecStr(x){
		  ...
		  if (s.length > 18){
		    // show first 10 + '…' + last 4 only if it has many decimals
		...
		function renderStakingReport(report){
		  ...
		  const renderPool = (row)=>{
		    ...
		    const t1 = row?.token1 || '';
		    const meta = row?.meta || '';
		    const poolId = row?.pool_id != null ? String(row.pool_id) : '—';
		    const pos = row?.positions != null ? String(row.positions) : '—';
		  ...
		  host.innerHTML = `
		      ...
		      <div class="top">
		        <div>
		          ...
		          <div class="sub">
		            ...
		            Every expedition’s CHIPS rewards are multiplied by <b>x${bonus.mult.toFixed(2)}</b>.
		          </div>
		          <div class="stk-note">
		            ${safe(bonusText)}
		          </div>
		        ...
		        <div style="display:flex;gap:8px;flex-wrap:wrap;justify-content:flex-end">
		          ...
		          <span class="pill">Targets: ${targets.length ? targets.map(safe).join(', ') : '—'}</span>
		        </div>
		      </div>
		    ...
		    <div class="staking-card">
		      <div class="title">
		        <span>🏦 Open staking positions (pools)</span>
		      ...
		      <div style="opacity:.9;font-size:12px;line-height:1.45;margin-bottom:8px">
		        ...
		        Increasing token amounts in these pools increases points → increases your <b>+% CHIPS per expedition</b>.
		      </div>
		    ...
		    <div class="staking-card">
		      ...
		      <div class="staking-kv">
		        ...
		        <div>
		          ...
		          <div class="v">${safe(report?.quote_token || '—')}</div>
		        </div>
		      </div>
		    </div>
		  `;
		...
		async function fetchAndRenderStakingReport({force=false} = {}){
		  ...
		  try { assertAuth(); } catch {
		    setStakingMeta({account:'—', startedAtUtc:null, cacheLabel:'—'});
		  ...
		  setStakingMeta({account:wax, startedAtUtc:null, cacheLabel: force ? 'refresh…' : 'miss…'});
		...
		function ensureStakingReportLoaded(){
		  fetchAndRenderStakingReport({force:false});
		    ...
		    function updateCapUI(){
		      ...
		      if (State.ui.reinfLabel){
		        State.ui.reinfLabel.textContent =
		          `Base ${base} + ${reinf}× Reinforcement (= ${extra} extra)`;
		      }
		      updateQuickPickButtons();
		    }
		    // ------------------------------------------------------------
		    // QUICK PICK: real cap (reinforcement + tired) + labels
		    // ------------------------------------------------------------
		    function getReadyCount(){
		      if (!Array.isArray(State.goblinsUser)) return 0;
		      return State.goblinsUser.filter(g => !isGoblinTired(g)).length;
		    }
		    function getRealCapForThisRun(){
		      const cap = getSendCap();
		      const ready = getReadyCount();
		      // “real maximum usable” = min(cap, ready). If ready unknown/0 but we have goblins, show cap-based fallback.
		      if (ready > 0) return Math.min(cap, ready);
		      return Math.min(cap, State.goblinsUser?.length || 0);
		    }
		    function updateQuickPickButtons(){
		      const cap = getSendCap();
		      const ready = getReadyCount();
		      const real = getRealCapForThisRun();
		      // Update text bits (safe if missing)
		      ...
		      s('qs-cap-n', cap);
		      s('qs-cap-n2', cap);
		      // If no goblins loaded yet, keep buttons readable
		      if (!State.nftsLoaded || !(State.goblinsUser?.length)){
		        s('qs-first-n', 0);
		        s('qs-best-n', 0);
		        s('qs-ready-n', 0);
		        s('qs-ready-n2', 0);
		    ...
		    function normalizeBestOrder(order){
		      const all = ['spd','loot','res','acc'];
		      const out = [];
		    ...
		    function readBestOrderFromUI(){
		      const v1 = document.getElementById('best-pri-1')?.value;
		      const v2 = document.getElementById('best-pri-2')?.value;
		      const v3 = document.getElementById('best-pri-3')?.value;
		      const v4 = document.getElementById('best-pri-4')?.value;
		      return normalizeBestOrder([v1,v2,v3,v4]);
		    }
		    function applyBestOrderToUI(order){
		      const o = normalizeBestOrder(order);
		    ...
		    function initBestPriorityControls(){
		      // set default values once
		      applyBestOrderToUI(State.runtime.bestPickOrder);
		      const onChange = ()=>{
		        State.runtime.bestPickOrder = readBestOrderFromUI();
		      };
		      ['best-pri-1','best-pri-2','best-pri-3','best-pri-4'].forEach(id=>{
		        const el = document.getElementById(id);
		        if (el) el.addEventListener('change', onChange);
		      });
		      const resetBtn = document.getElementById('best-pri-reset');
		      if (resetBtn){
		        resetBtn.addEventListener('click', ()=>{
		          State.runtime.bestPickOrder = ['spd','loot','res','acc'];
		          applyBestOrderToUI(State.runtime.bestPickOrder);
		          toast('Best pick priority reset to default.');
		        });
		      }
		    }
		    ...
		    function goblinStatsFromIndex(assetId){
		      const nft = State.nftIndex.get(String(assetId));
		      return {
		    ...
		    function bestPickScore(g, order){
		      const stats = goblinStatsFromIndex(g.asset_id);
		      const map = {
		        spd: toNumber(stats.spd),
		        loot: toNumber(stats.loot),
		        res: toNumber(stats.res),
		        acc: toNumber(stats.acc),
		      };
		      const o = normalizeBestOrder(order || State.runtime.bestPickOrder);
		      const weights = [1000, 100, 10, 1];
		      let weighted = 0;
		      for (let i=0;i<o.length;i++){
		        weighted += (map[o[i]] || 0) * weights[i];
		      }
		      const sum = map.spd + map.loot + map.res + map.acc;
		      const dp = toNumber(g.daily_power_num);
		      // return tuple-like for sorting
		      return { weighted, sum, dp };
		    }
		    function pickBestReadyGoblins(limit){
		      const cap = Math.max(0, toNumber(limit));
		      const order = State.runtime.bestPickOrder;
		      const ready = [...(State.goblinsUser || [])].filter(g => !isGoblinTired(g));
		      ready.sort((a,b)=>{
		        const A = bestPickScore(a, order);
		        const B = bestPickScore(b, order);
		        // 1) priority-weighted
		        if (B.weighted !== A.weighted) return B.weighted - A.weighted;
		        // 2) sum of all attributes
		        if (B.sum !== A.sum) return B.sum - A.sum;
		        // 3) daily power (nice fallback)
		    ...
		    function baseCostForThisGoblin(assetId){
		      // We don't have base-vs-final from backend without changing it.
		      // But we can explain: "Need" already includes resistance.
		      // We show: Needed power (final) + current DP and missing.
		      const id = String(assetId);
		      return toNumber(State.runtime.forecastPowerByAsset?.[id] || 0);
		    }
		    function updateDockUI(){
		    const cap = getSendCap();
		    const selArr = getSelectedGoblins();
		    const selected = selArr.length;
		    const tired = selArr.filter(isGoblinTired).length;
		    const ready = Math.max(0, selected - tired);
		  
		    const kSel = document.getElementById('kpi-selected');
		    const kCap = document.getElementById('kpi-cap');
		    const kReady = document.getElementById('kpi-ready');
		    const kTired = document.getElementById('kpi-tired');
		    const kEff = document.getElementById('kpi-eff');
		  
		    if (kSel) kSel.textContent = String(selected);
		    if (kCap) kCap.textContent = String(cap);
		    if (kReady) kReady.textContent = String(ready);
		    if (kTired) kTired.textContent = String(tired);
		  
		    const effSec =
		      toNumber(State.runtime.forecastData?.duration?.effective_seconds) ||
		      toNumber(State.runtime.forecastData?.duration_effective_seconds) ||
		      0;
		    if (kEff) kEff.textContent = effSec ? formatDurationMinSecFromSeconds(effSec) : '—';
		  
		    const btnRm = document.getElementById('dock-btn-remove-tired');
		    if (btnRm){
		      btnRm.textContent = `💤 Remove TIRED (${tired})`;
		      btnRm.disabled = tired === 0;
		    }
		  }
		    ...
		    function updateActionButtonsUI(){
		      ...
		      if (inProg){
		        btn.textContent = '⛏️ Expedition running…';
		      } else {
		...
		// ------------------------------------------------------------
		// UI OVERLAY (central notifications modal)
		// ------------------------------------------------------------
		const UINotify = (function(){
		  let root, card, titleEl, bodyEl, actionsEl, closeBtn;
		  let autoTimer = null;
		
		  function init(){
		    root = document.getElementById('ui-overlay');
		    if (!root) return;
		    card = root.querySelector('.ui-overlay__card');
		    titleEl = document.getElementById('ui-overlay-title');
		    bodyEl = document.getElementById('ui-overlay-body');
		    actionsEl = document.getElementById('ui-overlay-actions');
		    closeBtn = root.querySelector('.ui-overlay__close');
		  ...
		  function show(opts){
		    ...
		    const o = opts || {};
		    const ttl = String(o.title || 'Notice');
		    const text = o.text != null ? String(o.text) : '';
		    const html = o.html != null ? String(o.html) : '';
		
		    titleEl.textContent = ttl;
		
		    // body: prefer html if provided, else text
		    bodyEl.innerHTML = '';
		    if (html){
		    ...
		    actions.forEach(a=>{
		      ...
		      btn.type = 'button';
		      btn.className = `ui-overlay__btn ${a.variant || 'ghost'}`; // primary|ghost|danger
		    ...
		    if (!actions.length){
		      const btn = document.createElement('button');
		      btn.type = 'button';
		      btn.className = 'ui-overlay__btn primary';
		      btn.textContent = 'OK';
		    ...
		    // auto close after 10 seconds (unless disabled)
		    if (autoTimer){ clearTimeout(autoTimer); autoTimer = null; }
		    const autoMs = (o.autoCloseMs == null) ? 10000 : Number(o.autoCloseMs);
		    if (Number.isFinite(autoMs) && autoMs > 0){
		      autoTimer = setTimeout(()=> hide(), autoMs);
		    }
		  }
		...
		// ------------------------------------------------------------
		// toast() now routes to the central overlay
		// ------------------------------------------------------------
		function toast(msg){
		  UINotify.show({
		    title: 'Notice',
		    ...
		    async function requestForecast(duration, selArr){
		      ...
		      updateQuickPickButtons();
		      hydrateGoblinBuilder(); // così le card/badges “READY/TIRED” restano coerenti
		    ...
		    function renderForecastLoading(){
		      ...
		      if (realPanel){
		        realPanel.innerHTML = `
		          <div class="card">
		            <div class="hint">Computing forecast…</div>
		          </div>
		        `;
		      }
		      ...
		      if (!host.dataset.shellInitialized) {
				host.innerHTML = `
				  <div class="rf-shell">
				    <div class="rf-head">
				      <div>
				        <div class="rf-title">Run Forecast</div>
				        <div class="rf-sub">
				          Updates when you change <b>duration</b>, <b>selection</b>, or <b>simulator inputs</b>.
				          Numbers are shown with <b>max 3 decimals</b>.
				        </div>
				      </div>
				
				      <div class="rf-tabs" role="tablist" aria-label="Forecast mode">
				        <button type="button" data-mode="real" aria-pressed="true">🧬 My Goblins</button>
				        <button type="button" data-mode="sim" aria-pressed="false">🧪 Simulator</button>
				      </div>
				    </div>
				    ...
				    <div id="forecast-panel-sim" hidden>
				      <div class="rf-card">
		                ...
		                <label>
		                  ...
		                  <select id="sim-duration">
		                    ...
		                    <option value="60">1h — Long</option>
		                    <option value="120">2h — Marathon</option>
		                    <option value="360">6h — Endurance</option>
		                    <option value="1440">24h — Ultra</option>
		                  </select>
		                </label>
		                <label>
		                  Number of goblins
		              ...
		              <div id="forecast-sim-result" class="forecast-help" style="margin-top:8px;">
		                <div class="forecast-help-title">📘 Attribute-based simulator</div>
		                <p>
		    ...
		    function renderForecastRealContent(data, message){
				...
				return `
				  <div class="rf-grid">
				    <div class="rf-card">
				      <h4>Setup</h4>
				      <div class="rf-kv">
				        <div class="rf-row" title="Minutes chosen in the duration selector.">
				          <span class="k">Nominal duration</span>
				          <span class="v">${fmt3Smart(durNominal)} min</span>
				        </div>
				
				        <div class="rf-row" title="Estimated duration after the total SPEED bonus is applied.">
				          <span class="k">Effective duration</span>
				          <span class="v">${formatDurationMinSecFromSeconds(durEffSeconds)} · tier ${tierEff}m</span>
				        </div>
				
				        <div class="rf-row" title="Goblins actually used in the forecast (after applying the cap, if exceeded).">
				          <span class="k">Selected goblins</span>
				          <span class="v">${selectedCount}</span>
				        </div>
				      </div>
				
				      <div class="rf-badges">
				        <span class="rf-badge ${tiredCount ? 'warn' : 'ok'}">
				          💤 Tired: <b>${tiredCount}</b>
				        </span>
				        <span class="rf-badge ok">
				          ✅ Ready: <b>${Math.max(0, selectedCount - tiredCount)}</b>
				        </span>
				      </div>
				      ...
				      ${tiredCount ? `
				        <div class="rf-actions">
				          <button id="btn-remove-tired" class="btn-primary" type="button">
				            💤 Remove tired (${tiredCount})
				          </button>
				          <span style="font-size:12px;opacity:.85;">
				            Removes tired goblins from your current selection.
				          </span>
				        </div>
				      ` : ''}
				
				      <div class="rf-note">
				        <b>Tip:</b> Resistance reduces <b>Daily Power</b> cost per goblin. Speed reduces <b>effective duration</b>.
				      </div>
				    </div>
				    ...
				    <div class="rf-card">
				      <h4>Daily Power</h4>
				      <div class="rf-kv">
				        <div class="rf-row" title="Base daily cost per goblin BEFORE RESISTANCE is applied, for this duration tier.">
				          <span class="k">Nominal cost / goblin</span>
				          <span class="v">${fmt3Smart(baseNominalPerGoblin)} DP</span>
				        </div>
				
				        <div class="rf-row" title="Average effective daily cost per goblin AFTER RESISTANCE.">
				          <span class="k">Effective cost / goblin</span>
				          <span class="v">${selectedCount ? fmt3Smart(effAvgPerGoblin) : '—'} DP</span>
				        </div>
				
				        <div class="rf-row" title="Sum of all NOMINAL daily power costs (without RES) for this run.">
				          <span class="k">Total (nominal)</span>
				          <span class="v">${Number.isFinite(totalPowerNominal) ? fmt3Smart(totalPowerNominal) : '—'} DP</span>
				        </div>
				
				        <div class="rf-row" title="Sum of all EFFECTIVE daily power costs (with RES) for this run.">
				          <span class="k">Total (effective)</span>
				          <span class="v">${Number.isFinite(totalPowerEffective) ? fmt3Smart(totalPowerEffective) : '—'} DP</span>
				        </div>
				      </div>
				
				      <div class="rf-note">
				        “Need” on each goblin card already includes Resistance. If a goblin’s remaining Daily Power is below that value, it becomes <b>TIRED</b>.
				      </div>
				    </div>
				    ...
				    <div class="rf-card" style="grid-column:1/-1;">
				      <h4>Rewards</h4>
				      <div class="rf-kv">
				        <div class="rf-row" title="Expected CHIPS reward range for this run.">
				          <span class="k">Chips (min–max)</span>
				          <span class="v">${Number.isFinite(tMin) ? fmt3Smart(tMin) : '—'} – ${Number.isFinite(tMax) ? fmt3Smart(tMax) : '—'}</span>
				        </div>
				
				        ${Object.keys(probs).length ? `
				        <div class="rf-row" title="Relative probability of token reward tiers based on average ACC.">
				          <span class="k">Token tier probabilities</span>
				          <span class="v" style="font-family:inherit;font-weight:900;">
				            ${Object.entries(probs).map(([k,v]) => `${safe(k)} ${Math.round(v * 100)}%`).join(' · ')}
				          </span>
				        </div>` : ''}
				
				        ${nftRewards.length ? `
				        <div class="rf-row" title="Approximate chance of receiving each NFT template in a single run.">
				          <span class="k">NFT chances</span>
				          <span class="v" style="font-family:inherit;font-weight:900;">
				      ...
				      ${msgs.length ? `
				        <div class="rf-note">ℹ️ ${msgs.map(safe).join(' · ')}</div>
				      ` : ''}
				    </div>
				  </div>
				`;
		      ...
		      return `
		          ...
		          <div class="card">
		            <div class="kv">
		              <div title="Minutes chosen in the duration selector.">
		                <span class="label">⏱ Nominal duration</span>
		                <b>${durNominal} min</b>
		              </div>
		              ...
		              <div title="Base daily cost per goblin BEFORE RESISTANCE is applied, for this duration tier.">
		                <span class="label">💰 Nominal daily cost / goblin</span>
		                <b>
		              ...
		              <div title="Average effective daily cost per goblin AFTER RESISTANCE.">
		                <span class="label">🛡️ Effective daily cost / goblin</span>
		                <b>
		                  ${selectedCount ? formatDP(effAvgPerGoblin) : '—'} DP
		                  <span class="badge-inline">effective</span>
		                </b>
		              </div>
		              
		              <div title="Sum of all NOMINAL daily power costs (without RES) for this run.">
		                <span class="label">🔋 Total daily power (nominal)</span>
		                <b>${Number.isFinite(totalPowerNominal) ? formatDP(totalPowerNominal) : '—'} DP</b>
		              </div>
		              
		              <div title="Sum of all EFFECTIVE daily power costs (with RES) for this run.">
		                <span class="label">🔋 Total daily power (effective)</span>
		                <b>${Number.isFinite(totalPowerEffective) ? formatDP(totalPowerEffective) : '—'} DP</b>
		              </div>
		
		              <div title="Goblins actually used in the forecast (after applying the cap, if exceeded).">
		                <span class="label">🧬 Selected goblins</span>
		                <b>${selectedCount}</b>
		              </div>
		            </div>
		          </div>
		          ...
		          <div class="card">
		            <div class="kv">
		              <div title="Expected CHIPS reward range for this run.">
		                <span class="label">💎 Chips (min–max)</span>
		                <b>
		                  ${Number.isFinite(tMin) ? tMin : '—'} – ${Number.isFinite(tMax) ? tMax : '—'}
		                  <span class="badge-inline">reward</span>
		                </b>
		              </div>
		              ${Object.keys(probs).length ? `
		              <div title="Relative probability of token reward tiers based on average ACC.">
		                <span class="label">📊 Token tier probabilities</span>
		                <b>${
		              ...
		              <div title="Approximate chance of receiving each NFT template in a single run.">
		                <span class="label">🎁 NFT drop chances</span>
		                <b>${
		                  nftRewards
		              ...
		              <div style="margin-top:6px;">
		                <button id="btn-remove-tired" class="btn-primary" type="button">
		                  💤 Remove ${tiredCount} tired goblin${tiredCount > 1 ? 's' : ''} from selection
		                </button>
		              </div>` : ''}
		            </div>
		          </div>
		        ...
		        <div class="card forecast-help">
		          <div class="forecast-help-title">📘 How your attributes affect this run</div>
		          <ul>
		        ...
		        ${msgs.length ? `<div class="hint">ℹ️ ${msgs.map(safe).join(' · ')}</div>` : ''}
		      `;
		    }
		    ...
		    function updateForecastSimResult() {
		      ...
		      // Nessun risultato ancora → testo introduttivo
		      if (!sim) {
		        host.innerHTML = `
		          <div class="forecast-help-title">📘 Attribute-based simulator</div>
		          <p>
		      ...
		      host.innerHTML = `
		        <div class="forecast-help-title">🧪 Simulation result</div>
		    ...
		    async function runSimulationFromForm() {
		      ...
		      host.innerHTML = `
		        <div class="forecast-help-title">🧪 Attribute-based simulator</div>
		        <p>Calculating a sample expedition with your numbers…</p>
		      `;
		      ...
		      // 👇 Qui usiamo direttamente la struttura del backend, che ha:
		    ...
		    function wireRemoveTiredButton(){
		      ...
		      btn.onclick = () => {
		        ...
		        hydrateGoblinBuilder();
					UINotify.show({
					  title: 'Tired goblins removed',
					  html: `
					    <p><strong>${before - after}</strong> goblin(s) removed from your selection.</p>
					    <p style="opacity:.9">Tip: choose a shorter duration to include more goblins.</p>
					  `,
					  actions: [
		...
		drawDropChest(ctx){
		  ...
		  // hologram scanlines (senza “rompere” alpha globale)
		  if (phase === 'hologram'){
		    const t = clamp(
		  ...
		  if (Assets.chest){
		    ...
		    // ✅ VARIANTE "più bella": idle ping-pong 0-1-2-1
		    let fi = 0;
		  ...
		  this.updateDropParticles(ctx, p.x, p.y, size);
		    ...
		    function recalcMyExpeditionFlagAndUI(){
		      ...
		      if (!mine) scheduleForecastIfReady();
				renderMyExpeditionOverlay();
				startMyOverlayTicker();
				
		    }
		function getMyActiveExpedition(){
		  const me = (State.user.wax_account || '').trim().toLowerCase();
		  if (!me) return null;
		
		  const exps = State.globalExpeditions || [];
		  for (const e of exps){
		    const owner = String(e.wax_account || e.owner || '').trim().toLowerCase();
		    if (owner && owner === me) return e;
		  }
		  return null;
		}
		
		function formatAttrs(exp){
		  // prova campi “probabili” senza rompere se non esistono
		  const pick = (k1, k2, k3) => {
		    const v = exp?.[k1] ?? exp?.[k2] ?? exp?.[k3];
		    const n = Number(v);
		    return Number.isFinite(n) ? n : null;
		  };
		
		  const speed = pick('total_speed','speed_total','speed');
		  const res   = pick('total_resistance','resistance_total','resistance');
		  const loot  = pick('total_loot','total_loot_hungry','loot_hungry_total');
		  const acc   = pick('total_accuracy','accuracy_total','accuracy');
		
		  const rows = [];
		  if (speed != null) rows.push({ k:'⚡ SPEED', v: speed });
		  if (res   != null) rows.push({ k:'🛡️ RES',   v: res });
		  if (loot  != null) rows.push({ k:'💎 LOOT',  v: loot });
		  if (acc   != null) rows.push({ k:'🎯 ACC',   v: acc });
		
		  return rows;
		}
		State.ui = State.ui || {};
		State.ui.myOverlayCollapsed = State.ui.myOverlayCollapsed ?? false;
		
		function renderMyExpeditionOverlay(){
		  const host = document.getElementById('my-exp-overlay');
		  if (!host) return;
		
		  const exp = getMyActiveExpedition();
		  if (!exp){
		  ...
		  // ---- dati (usi le tue funzioni già presenti) ----
		  const sr = fromSecondsRemaining(exp); // tua funzione
		  const leftMs = sr ? sr.leftMs(Date.now()) : getTimeLeftMs(exp, new Date());
		  const endAt  = sr ? sr.endsAt() : getEndsAt(exp);
		
		  const left = (leftMs != null) ? fmtHMS(leftMs) : '--:--:--';
		  const tz = Intl.DateTimeFormat().resolvedOptions().timeZone || 'local';
		  const endLabel = endAt
		    ? endAt.toLocaleString(undefined, { hour:'2-digit', minute:'2-digit', second:'2-digit' }) + ` (${tz})`
		    : '—';
		
		  const nominalMin = Number(exp.nominal_minutes ?? exp.duration_minutes ?? 0) || 0;
		  const gcount =
		    Number(exp.total_goblins ?? exp.goblins_count ?? 0) ||
		    (Array.isArray(exp.goblin_ids) ? exp.goblin_ids.length : 0);
		
		  const owner = safe(exp.wax_account || exp.owner || '');
		
		  const collapsed = !!State.ui.myOverlayCollapsed;
			// ✅ responsive sizing (mobile-safe)
			const vw = Math.max(320, window.innerWidth || 360);
			const vh = Math.max(480, window.innerHeight || 640);
			
			// pannello stretto su mobile, un po' più largo su desktop
			const panelW = Math.min(320, Math.max(240, Math.floor(vw * 0.72))); // 240..320
			const collapsedW = 28;
			
			// altezza massima: non deve mai uscire dallo schermo
			const panelMaxH = Math.floor(vh - 24);
			const panelStyle = `
			  position: relative;
			  width: 100%;
			  max-height: ${panelMaxH}px;
			  overflow: hidden;
			
			  border-radius: ${collapsed ? '999px' : '16px'};
			  background: ${collapsed
			    ? 'linear-gradient(180deg, rgba(245,245,245,0.88), rgba(170,170,170,0.70))'
			    : 'linear-gradient(180deg, rgba(7,12,18,.86), rgba(7,12,18,.72))'
			  };
			  border: 1px solid ${collapsed ? 'rgba(255,255,255,0.70)' : 'rgba(255,255,255,.10)'};
			...
			host.innerHTML = `
			  <div style="
			    ...
			    width: ${collapsed ? collapsedW : panelW}px;
			    max-width: calc(100vw - 16px - env(safe-area-inset-right));
			    pointer-events: auto;
			  ">
			    <div id="myExpPanel" style="${panelStyle}">
			      
			      <!-- HANDLE (sempre visibile) -->
			      <div id="myExpHandle" title="${collapsed ? 'Expand' : 'Collapse'}" style="
			        position:absolute;
			        top:0;
			        bottom:0;
			        right:0;
			        ...
			        user-select:none;
			        background: linear-gradient(180deg, rgba(245,245,245,0.92), rgba(170,170,170,0.70));
			      ...
			      <!-- SCROLL AREA (solo quando espanso) -->
			      <div id="myExpScroll" style="
			        padding: 12px 12px 12px 12px;
			        padding-right: ${collapsedW + 10}px;
			      ...
			      ">
			        <div style="display:flex; align-items:flex-start; justify-content:space-between; gap:10px;">
			          <div style="min-width:0;">
			            <div style="font-size:11px; letter-spacing:.10em; text-transform:uppercase; opacity:.8;">
			              My Expedition
			            </div>
			          ...
			          <button id="myExpToggleBtn" type="button" style="
			        ...
			        <div style="
			          ...
			          color: rgba(214,255,232,.95);
			          font-weight:950;
			          font-size:12px;
			          line-height: 1.2;
			        ">
			          Expedition running<br>
			          <span style="opacity:.92;">to finish:</span>
			          <span id="myExpLeft" style="font-size:13px;">${left}</span>
			        </div>
			
			        <div style="margin-top:10px; display:grid; gap:8px;">
			          <div style="padding:10px; border-radius:12px; background: rgba(255,255,255,.06); border:1px solid rgba(255,255,255,.08);">
			            <div style="font-size:11px; opacity:.8;">Ends (local)</div>
			            <div style="font-size:13px; font-weight:950; margin-top:4px; line-height:1.2;">
			              <span id="myExpEnd">${endLabel}</span>
			            </div>
			          </div>
			
			          <div style="display:grid; grid-template-columns: 1fr 1fr; gap:8px;">
			            <div style="padding:10px; border-radius:12px; background: rgba(255,255,255,.06); border:1px solid rgba(255,255,255,.08);">
			              <div style="font-size:11px; opacity:.8;">Nominal</div>
			              <div style="font-size:15px; font-weight:950; margin-top:2px;">
			                ${nominalMin ? `${nominalMin} min` : '—'}
			              </div>
			            </div>
			            <div style="padding:10px; border-radius:12px; background: rgba(255,255,255,.06); border:1px solid rgba(255,255,255,.08);">
			              <div style="font-size:11px; opacity:.8;">Goblins</div>
			              <div style="font-size:15px; font-weight:950; margin-top:2px;">${gcount || 0}</div>
			            </div>
			          </div>
			        </div>
			...
			`;
		  // ---- binding toggle (senza duplicare listener) ----
		  const handle = host.querySelector('#myExpHandle');
		  const btn = host.querySelector('#myExpToggleBtn');
		
		  const toggle = (ev)=>{
		    ev?.preventDefault?.();
		    ev?.stopPropagation?.();
		    State.ui.myOverlayCollapsed = !State.ui.myOverlayCollapsed;
		    renderMyExpeditionOverlay();
		  };
		
		  if (handle) handle.onclick = toggle;
		  if (btn) btn.onclick = toggle;
		}
		
		
		// tick “leggero”: aggiorna solo il countdown senza rifare tutto quando possibile
		function startMyOverlayTicker(){
		  if (State.runtime._myOverlayTicking) return;
		  State.runtime._myOverlayTicking = true;
		
		  // evita doppi timer se viene richiamata più volte
		  if (State.runtime._myOverlayInterval) clearInterval(State.runtime._myOverlayInterval);
		
		  State.runtime._myOverlayInterval = setInterval(() => {
		    const host = document.getElementById('my-exp-overlay');
		    ...
		    // stesso calcolo usato nel render (v9)
		    ...
		    // ✅ aggiorna SOLO testo (non innerHTML)
		    const leftEl = host.querySelector('#myExpLeft');
		    if (leftEl) leftEl.textContent = left;
		
		    const endEl = host.querySelector('#myExpEnd');
		    if (endEl) endEl.textContent = endLabel;
		
		  }, 250); // 4 volte/sec: fluido e non rompe i click
		}
		      ...
		      exps.forEach(exp=>{
		        ...
		        right.innerHTML =
		          `<div style="font-weight:900">
		             <span class="badge ok">⏳ <span data-eta>${leftMs!=null?fmtHMS(leftMs):'--:--:--'}</span></span>
		           </div>
		...
		function updateTicker(){
		  ...
		  if (ev){
		    ...
		    if (ev.event_type === 'drop' || ev.event_type === 'drop_claimed'){
		      ...
		      if (st === 'spawned' && ev.event_type === 'drop'){
		        el.innerHTML =
		          `<span class="ticker-time">[${when}]</span> ` +
		          `✨ <strong>A chest has spawned!</strong> ` +
		      ...
		      if (st === 'claimed' || ev.event_type === 'drop_claimed'){
		        ...
		        const rewardHtml = hasAmt
		          ? `→ <span class="ticker-rew">+${formatCompact(amtNum)} ${sym}</span>`
		          : '';
		      
		        const txHtml = txid
		          ? ` <span class="ticker-extra" title="${safe(txid)}">tx: ${safe(txid.slice(0, 10))}…</span>`
		          : '';
		      
		        el.innerHTML =
		          `<span class="ticker-time">[${when}]</span> ` +
		          `🏴‍☠️ <strong>${by}</strong> claimed a chest ` +
		          rewardHtml +
		          txHtml +
		      ...
		      if (st === 'expired'){
		        el.innerHTML =
		          `<span class="ticker-time">[${when}]</span> ` +
		          `⌛ <strong>A chest expired.</strong> ` +
		          `<span class="ticker-eff">Stay active in chat—new chests can spawn anytime.</span>`;
		        return;
		      }
		    }
		  ...
		  el.textContent = 'Waiting for events…';
		    ...
		    function buildDurationPresetButtons(){
		      const host = document.getElementById('dock-duration');
		      const sel = document.getElementById('sel-duration');
		      const hint = document.getElementById('dock-duration-hint');
		      if (!host || !sel) return;
		    
		      host.innerHTML = '';
		      const opts = [...sel.options].map(o=>({
		        value: String(o.value),
		        label: String(o.textContent || o.label || o.value)
		      }));
		    
		      function setActive(v){
		        host.querySelectorAll('.dur-btn').forEach(b=>{
		      ...
		      opts.forEach(o=>{
		        ...
		        btn.textContent = o.label.replace(' — ', ' · ');
		        btn.addEventListener('click', ()=>{
		          sel.value = o.value;
		      ...
		      setActive(sel.value);
		      // Keep original hint text but mirror it here
		      if (hint){
		        hint.textContent = 'Pick a preset. Resistance reduces Daily Power cost per goblin.';
		      }
		    }
		    ...
		    function mountBuilderControls(){
		      ...
		      buildDurationPresetButtons();
		      // Optional: hide the old select UI in the dock area (logic still uses it)
		      const durSwitch = document.getElementById('duration-switch');
		      if (durSwitch) durSwitch.style.display = 'none';
		      if (hintBox){
		    ...
		    function hydrateGoblinBuilder(){
		      ...
		      let filtered = applyFilters(State.goblinsUser);
		      // status pill filter
		      const mode = State.runtime.listMode || 'all';
		      if (mode === 'selected'){
		        filtered = filtered.filter(g => State.selectedGoblinIds.has(g.asset_id));
		      } else if (mode === 'ready'){
		        filtered = filtered.filter(g => !isGoblinTired(g));
		      } else if (mode === 'tired'){
		        filtered = filtered.filter(g => isGoblinTired(g));
		      }
		      // sorting
		      const sortMode = State.runtime.sortMode || 'best';
		      filtered.sort((a,b)=>{
		        const aid = String(a.asset_id), bid = String(b.asset_id);
		        const aT = isGoblinTired(a) ? 1 : 0;
		        const bT = isGoblinTired(b) ? 1 : 0;
		        // BEST: READY first, then smallest missing, then highest DP
		        if (sortMode === 'best'){
		          if (aT !== bT) return aT - bT; // ready first
		          const aNeed = toNumber(State.runtime.forecastPowerByAsset?.[aid] || 0);
		          const bNeed = toNumber(State.runtime.forecastPowerByAsset?.[bid] || 0);
		          const aHave = toNumber(a.daily_power_num || 0);
		          const bHave = toNumber(b.daily_power_num || 0);
		          const aMiss = Math.max(0, aNeed - aHave);
		          const bMiss = Math.max(0, bNeed - bHave);
		          if (aMiss !== bMiss) return aMiss - bMiss;
		          return bHave - aHave;
		        }
		        if (sortMode === 'dp'){
		          return toNumber(b.daily_power_num || 0) - toNumber(a.daily_power_num || 0);
		        }
		        const aStats = goblinStatsFromIndex(a.asset_id);
		        const bStats = goblinStatsFromIndex(b.asset_id);
		        if (sortMode === 'loot') return toNumber(bStats.loot) - toNumber(aStats.loot);
		        if (sortMode === 'res')  return toNumber(bStats.res)  - toNumber(aStats.res);
		        if (sortMode === 'spd')  return toNumber(bStats.spd)  - toNumber(aStats.spd);
		        if (sortMode === 'acc')  return toNumber(bStats.acc)  - toNumber(aStats.acc);
		        return 0;
		      });
		      const cap = getSendCap();
		      const COLLAPSED = 30;
		      ...
		      visible.forEach(g=>{
		        ...
		        const stats = goblinStatsFromIndex(id);
		        
		        const have = toNumber(g.daily_power_num || 0);
		        const need = toNumber(State.runtime.forecastPowerByAsset?.[id] || 0);
		        const missing = need ? Math.max(0, need - have) : 0;
		        
		        const expanded = (State.runtime.openDetailsId === id);
		        
		        let statusLabel = tired ? 'TIRED' : (sel ? 'SELECTED' : 'READY');
		        ...
		        card.innerHTML = `
		          ...
		          <div class="gob-math">
		            ...
		            <div class="math-box"><div class="k">Need</div><div class="v">${need ? formatDP(need) : '—'}</div></div>
		            <div class="math-box"><div class="k">Missing</div><div class="v">${need ? formatDP(missing) : '—'}</div></div>
		          </div>
		        
		          <div class="gob-stats" aria-label="Attributes">
		            <span class="stat-chip"><span class="lbl">LOOT</span><span class="val">${formatCompact(stats.loot)}</span></span>
		            <span class="stat-chip"><span class="lbl">SPD</span><span class="val">${formatCompact(stats.spd)}</span></span>
		            <span class="stat-chip"><span class="lbl">RES</span><span class="val">${formatCompact(stats.res)}</span></span>
		            <span class="stat-chip"><span class="lbl">ACC</span><span class="val">${formatCompact(stats.acc)}</span></span>
		          </div>
		        
		          ${expanded ? `
		            <div class="gob-details">
		              <div><b>What this means:</b></div>
		              <div style="margin-top:6px;">
		                <span style="opacity:.92">Need</span> is the <b>final Daily Power cost</b> for this duration (Resistance already applied).
		                If a goblin is <b>TIRED</b>, its remaining Daily Power is below the required cost.
		              </div>
		              <div class="details-row">
		                <div class="mini">Duration preset: <b>${formatMinutesShort(State.expedition.durationMinutes)}</b></div>
		                <div class="mini">Daily Power remaining: <b>${formatDP(have)}</b></div>
		                <div class="mini">Cost (final): <b>${need ? formatDP(need) : '—'}</b></div>
		              </div>
		            </div>
		          ` : ``}
		        `;
		        if (!tired){
		          card.addEventListener('click', (e)=>{
		            ...
		            if (e.altKey){
		              State.runtime.openDetailsId = (State.runtime.openDetailsId === id) ? null : id;
		              hydrateGoblinBuilder();
		              return;
		            }
		    ...
		    function wireBuilderUI(){
		      ...
		      const btnAuto = document.getElementById('dock-btn-autopick');
		      if (btnAuto){
		        btnAuto.addEventListener('click', ()=>{
		          if (!State.nftsLoaded) return;
		          const cap = getSendCap();
		          // Best for run: READY first, then highest Daily Power
		          const eligible = [...State.goblinsUser]
		            .filter(g => !isGoblinTired(g))
		            .sort((a,b)=> toNumber(b.daily_power_num) - toNumber(a.daily_power_num));
		          for (const g of eligible){
		            if (State.selectedGoblinIds.size >= cap) break;
		      ...
		      if (btnRm){
		        btnRm.addEventListener('click', ()=>{
		          const tiredIds = getSelectedGoblins().filter(isGoblinTired).map(g=> g.asset_id);
		          tiredIds.forEach(id=> State.selectedGoblinIds.delete(id));
		          hydrateGoblinBuilder();
		        });
		      }
		      // status pills
		      const pills = document.getElementById('flt-pills');
		      if (pills){
		        pills.querySelectorAll('.pill').forEach(btn=>{
		          btn.addEventListener('click', ()=>{
		            pills.querySelectorAll('.pill').forEach(x=> x.classList.remove('is-on'));
		            btn.classList.add('is-on');
		            State.runtime.listMode = btn.dataset.mode || 'all';
		            hydrateGoblinBuilder();
		          });
		        });
		      }
		      ...
		      if (fltSort){
		        fltSort.addEventListener('change', ()=>{
		          State.runtime.sortMode = fltSort.value || 'best';
		          hydrateGoblinBuilder();
		        });
		      }
		      if (State.ui.btnFirst) State.ui.btnFirst.onclick = ()=>{
		        ...
		        const cap = getSendCap();
		        const ready = State.goblinsUser.filter(g => !isGoblinTired(g));
		        const real = getRealCapForThisRun(); // cap ∩ ready
		      
		        // Pick first READY up to real cap
		        ready.slice(0, real).forEach(g=> State.selectedGoblinIds.add(g.asset_id));
		      
		        // Fallback (if forecast not ready yet / zero ready)
		        if (State.selectedGoblinIds.size === 0 && State.goblinsUser.length){
		          State.goblinsUser.slice(0, Math.min(cap, State.goblinsUser.length))
		            .forEach(g=> State.selectedGoblinIds.add(g.asset_id));
		          toast('Forecast not ready (or no ready goblins). Picked from your full list.');
		        }
		      ...
		      if (State.ui.btnBest) State.ui.btnBest.onclick = ()=>{
		        ...
		        // Pick BEST READY using priority order (sum + priority weighting)
		        const chosen = pickBestReadyGoblins(real);
		        chosen.forEach(g=> State.selectedGoblinIds.add(g.asset_id));
		      
		        // Fallback if zero ready (or forecast missing)
		        if (State.selectedGoblinIds.size === 0 && State.goblinsUser.length){
		          // as last resort: best by DP only, from full list
		          [...State.goblinsUser]
		            .sort((a,b)=>(toNumber(b.daily_power_num))-(toNumber(a.daily_power_num)))
		            .slice(0, Math.min(cap, State.goblinsUser.length))
		            .forEach(g=> State.selectedGoblinIds.add(g.asset_id));
		          toast('No ready goblins for this duration. Picked best by Daily Power as fallback.');
		        }
		    ...
		    async function fetchUserNFTs() {
		      ...
		      if (btn && !btn.dataset.loading) {
		        ...
		        btn.textContent = '⏳ Loading goblins…';
		      }
		      if (statusEl) {
		        statusEl.textContent = 'Loading goblins from the atomicassets…';
		      }
		      ...
		      try {
		        ...
		        if (btn) {
		          btn.disabled = false;
		          btn.textContent = '🔄 Load / Refresh goblins';
		      ...
		      // nfts è sempre dentro payload.nfts col nuovo backend
		      const arr = Array.isArray(payload.nfts)
		        ? payload.nfts
		        // fallback ultra difensivo se in futuro tornasse un array “nudo”
		      ...
		      if (btn) {
		        btn.disabled = false;
		        btn.textContent = '🔄 Refresh goblins';
		        delete btn.dataset.loading;
		      }
		    }
		    ...
		    async function sendExpedition(){
		      ...
		      // Se sei nel tab Simulator, ricordiamo che il run è sempre sui goblin reali
		      ...
		      toast(`⛏️ Expedition started! ${ids.length}/${cap} goblins · ETA ${fmtHMS(durSec*1000)} (ends ${timeHM(endAt)})`);
				// If backend had to skip extra tired goblins (race condition), show an explicit modal
				const skippedByBackend = (res.data && Array.isArray(res.data.skipped_tired)) ? res.data.skipped_tired : [];
				if (skippedByBackend.length){
				  UINotify.show({
				    title: 'Some goblins were too tired',
				    html: `
				      <p>The expedition started, but <strong>${skippedByBackend.length}</strong> goblin(s) had insufficient daily power at the moment of start.</p>
				      <p style="margin-top:10px;opacity:.95">Skipped IDs:</p>
				      <p><code>${skippedByBackend.map(x=>String(x)).join(', ')}</code></p>
				      <p style="opacity:.9">You can remove them from your current selection to avoid confusion.</p>
				    `,
				    actions: [
				      {
				        label: 'Remove skipped from selection',
				        variant: 'primary',
				        onClick: ()=>{
				          for (const id of skippedByBackend) State.selectedGoblinIds.delete(String(id));
				          hydrateGoblinBuilder();
				        }
				      },
				      { label:'Keep selection', variant:'ghost' }
				    ],
				    autoCloseMs: 10000
				  });
				}
				
		      pushCanvasMsg(`⛏️ ${State.user.wax_account} started an expedition with ${ids.length}/${cap} goblins · ETA ${fmtHMS(durSec*1000)}`);
			...
			async function claimActiveDrop(){
			  ...
			  // Se non ok o 409 => qualcuno ha già claimato / scaduto
			  if (!res.ok || res.status === 409 || res.data.ok === false) {
			    ...
			    toast(`Too late — claimed by ${by}.`);
			    pushCanvasMsg(`💨 Chest claimed by ${by}.`);
			  ...
			  if (amtNum != null && Number.isFinite(amtNum)) {
			    toast(`🎉 You found +${formatCompact(amtNum)} ${sym} in the chest!`);
			  } else {
			    toast('🎉 Chest claimed!');
			  }
			
			  // ticker local bello (per qualche secondo), include amount perché arriva da /2drop_claim
			  const when = timeHM(new Date());
			  State.drop.lastLocalTickerHtml =
			    `<span class="ticker-time">[${when}]</span> ` +
			    `🏴‍☠️ <strong>${safe(claimedBy)}</strong> claimed a chest ` +
			    (amtNum != null && Number.isFinite(amtNum)
			      ? `→ <span class="ticker-rew">+${formatCompact(amtNum)} ${safe(sym)}</span>`
			      : '') +
			    `<span class="ticker-eff">Use Twitch chat commands to spawn more chests and win prizes.</span>`;
			  State.drop.lastLocalTickerUntilMs = Date.now() + 12_000;
			  updateTicker();
			
			// ✅ NON sovrascrivere con res.data (manca x/y), tieni posizione e marca claimed
			...
			window.claimActiveDrop = claimActiveDrop;
		    ...
		    async function fetchGlobalSnapshot(){
		      ...
		      try {
		        ...
		        if (expRes.ok && expRes.data){
		          ...
		          // ✅ FIX: read "expeditions" first (your backend shape)
		        ...
		        if (dropRes && dropRes.ok && dropRes.data) {
		          ...
		          if (normalized && nextKey && nextKey !== prevKey) {
		            ...
		            State.drop.fx.particles = [];
		            State.drop.fx.shockwaves = [];
		            pushCanvasMsg(`🎁 A chest has appeared!`);
		          }
				  ...
				  if (State.engine && typeof State.engine.setDrop === 'function') {
					  State.engine.setDrop(normalized);
				  }
		        }
		  ...
		  host.style.cssText = [
		    ...
		    function mount(){
		      ...
		      State.ui.btnStakingCopy = document.getElementById('btn-staking-copy');
			    initMyExpOverlayHost();
		      applyOverlayMode();
		      setTabsBehavior();
		      try { initBestPriorityControls(); } catch {}
		      try { updateQuickPickButtons(); } catch {}
		      wireBuilderUI();
		      updateCapUI();
		      initDurationSwitch();
		      mountBuilderControls();
		      updateDockUI();
		      setBuilderEnabled(false);      // finché non carichi i goblin
		      updateGoblinGlobalStats();     // 0,0,0 all'inizio
		      updateActionButtonsUI();
		      scheduleForecastIfReady();     // mostra solo messaggio “Load your goblins…”
		        ...
		        State.ui.btnStakingCopy.onclick = ()=>{
		          try{
		            ...
		            const text =
		              ...
		              Generated (UTC): ${data?.started_at_utc || '—'}
		              Tip: Add more liquidity to your staking pools to increase points → increase bonus %.`;
		            if (navigator.clipboard?.writeText){
		              navigator.clipboard.writeText(text)
		      ...
		      fetchGlobalSnapshot();
		      startPolling();
		  	  // ✅ ticker rotator: cambia evento ogni 2 secondi
		    ...
		    window.__GOBLIN_DEX_STATE__ = State;
		    window.State = State;
		    window.claimActiveDrop = claimActiveDrop;
		  })();
